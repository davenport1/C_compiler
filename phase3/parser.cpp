...
/**
 * Helper functions for output testing
 * 
 */
// static string specifierType(int token) {
// 	if(token == INT) return "INT";
// 	else if(token == VOID) return "VOID";
// 	else if(token == CHAR) return "CHAR";
// 	else if(token == LONG) return "LONG";
// 	else exit(ERROR);
// }

// static void outputType(string spec, string id, int arrlen, unsigned indirection, string decl) {
// 	if(arrlen == 0) 
// 	cout << id << ": (" << spec << ", " << indirection << ", " << decl << ")" << endl;
// 	else
// 	cout << id << ": " << arrlen << "(" << spec << ", " << indirection << ", " << decl << ")" << endl;
// }
...
static void declarator(int typespec)
{
	...
	unsigned long arrnum = stoul(num);

	//outputType(specifierType(typespec), id, arrnum, indirection, "ARRAY");
	declareVariable(id, Type(typespec, indirection, arrnum));
    } else {
		//outputType(specifierType(typespec), id, 0, indirection, "SCALAR");
...
{	
	int typespec;
	Parameters params;
    if (lookahead == VOID) {	//this is where void error is happening
	typespec = specifier();
	...
	string id = lexbuf;
    match(ID);
	Type temp(typespec, indirection);
	Type *param = &temp;
    ...
    while (lookahead == ',') {
	match(',');
	Type moreParam = parameter();
	Type *paramPtr = &moreParam;
...
static void globalOrFunction()
{
	...
	if (lookahead == ')') {
	    match(')');
		declareFunction(id, Type(typespec, indirection, nullptr));
		//outputType(specifierType(typespec), id, 0, indirection, "FUNCTION");
	    remainingDeclarators(typespec);
